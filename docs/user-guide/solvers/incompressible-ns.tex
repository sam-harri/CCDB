...
The implicit solver is based on the work of \cite{Dong2010,Simo1994}. Following the 
...
is an approximate advection velocity. The Advection velocity $\tilde{\mathbf{u}}$ can 
take either the form described in \cite{Simo1994} or \cite{Dong2010} which are defined as,
\begin{align}
    \texttt{Simo}\qquad\tilde{\mathbf{u}}^{n+1} &=
    \sum_q \frac{\alpha_q}{\gamma} \mathbf{u}^{n-q},\\
    \texttt{Dong}\qquad\tilde{\mathbf{u}}^{n+1} &=
    \sum_q \frac{\alpha_q}{\gamma} \mathbf{u}^{n-q} 
    ...
    - \mathbf{f}^{n+1} \right).
\end{align}
The scheme defaults to \texttt{Simo}, however, can be chosen via the \inltt{SolverInfo} with either

\begin{lstlisting}[style=XMLStyle]
<I PROPERTY="AdvectionVelocity"  VALUE="Simo"/>
<I PROPERTY="AdvectionVelocity"  VALUE="Dong"/>
\end{lstlisting}
